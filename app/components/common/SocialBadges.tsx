/**
 * The social badges (top-left), kept from the previous site.
 *
 * Each icon is circular *in the artwork*: a full-bleed disc whose viewBox is
 * cropped to the disc's own bounds, so all three paint the same circle in the
 * same 26px box. There is no CSS circle behind them and there must not be one.
 * Pure black/white only; brand colours were tried and reverted.
 */

/** GitHub's Invertocat (Simple Icons), black on a white disc. */
const GithubIcon = () => (
  <svg className="social-icon" viewBox="0 0 32 32" aria-hidden="true">
...
/**
 * LinkedIn's circular badge (Entypo Social `linkedin-with-circle`) in white.
 * Its "in" is a knockout, so the black disc behind shows through the letters.
 */
const LinkedinIcon = () => (
  <svg className="social-icon" viewBox="0.4 0.4 19.2 19.2" aria-hidden="true">
...
/** The Gmail mark on a white disc, its five colours flattened to black. */
const EmailIcon = () => (
...
const SOCIAL_LINKS: SocialLink[] = [
  { href: 'https://github.com/ashrafjr-n', label: 'GitHub: ashrafjr-n', Icon: GithubIcon, text: 'ashrafjr-n' },
  { href: 'https://www.linkedin.com/in/ashraf-al-jarabeah-a94509408/', label: 'LinkedIn', Icon: LinkedinIcon },
  { href: 'mailto:aannaelj@gmail.com', label: 'Email', Icon: EmailIcon },
];

const SocialBadges = () => (
  <div className="social-badges">
    {SOCIAL_LINKS.map(({ href, label, Icon, text }) => {
      // mailto: has nowhere to open a tab; the two outbound links do.
      const external = !href.startsWith('mailto:');
      return (
        <a key={href} className="social-badge" href={href} aria-label={label}
