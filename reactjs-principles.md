...
- Show explicit loading, error, and empty states for anything async or
  filterable — never let the UI silently render nothing or freeze with no feedback.
...
- Every interactive element must be reachable and operable by keyboard alone (tab
  order, Enter/Space activation, visible focus state) — don't rely on mouse-only
  events like `onMouseOver` for anything essential.
- Form inputs always have an associated `label` (or `aria-label`) — never a
  placeholder as the only identifier.
- Manage focus on route/view changes and after closing a modal/dialog — don't leave
  focus stranded on a removed element.

## 13. AI Engineering Rules — how to work, not just what to write

These govern behavior *while* writing code, not the code's final shape. A change
that follows every rule above but ignores these can still be a bad change.

- **Inspect before deciding.** Look at the existing codebase — folder structure,
  existing components/hooks/utilities, naming patterns — before creating anything
  new. Never assume a file, component, API, or dependency exists or doesn't exist;
  check first.
- **Reuse before creating.** If a component, hook, or utility that does this (or
  close to it) already exists, extend or reuse it instead of writing a parallel one.
- **Smallest change that satisfies the request.** Touch the minimum number of files
  needed. A small feature request should not turn into a restructuring of unrelated
  files — that belongs to an explicit hygiene/refactor pass, not a feature command.
- **Don't over-engineer.** Don't introduce a new abstraction, hook, context,
  service, or dependency without a concrete, current reason. Build for the
  requirement in front of you, not a hypothetical future one.
- **Preserve existing behavior.** A change must not silently break another feature.
  Before deleting or changing shared code, check where else it's used.
- **No fake completion.** Don't leave `TODO`s, mock data, hard-coded placeholders,
  or stubbed-out logic and call the task done — unless the task explicitly asked
  for a stub/placeholder.
- **Don't hide problems.** Never silence an error with a bare `any`, an empty
  `catch` block, a suppressed TypeScript error, or a disabled lint rule just to
  make a warning disappear. Fix the actual issue or surface it.
- **Validate before calling it done.** After a change, run the project's
  typecheck/lint/build/tests if available. "It looks right" is not the same as "it
  passes." A task isn't finished until it's verified.
- **Keep config changes minimal and justified.** Don't touch `package.json`,
  `tsconfig`, ESLint/Prettier config, or the bundler config unless the task
  actually requires it — and say so when you do.
- **When ambiguous, follow existing convention over inventing a new one.** If the
  codebase already has a pattern for something similar, match it rather than
  introducing a stylistically different approach.